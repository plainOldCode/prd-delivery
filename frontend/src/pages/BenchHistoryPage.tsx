...
/* ---------- Row ---------- */
function HistoryRow({ run, isSelected, onToggle }: { run: BenchHistoryRow; isSelected: boolean; onToggle: (id: number) => void }) {
...
/* ---------- Compare Panel ---------- */
function ComparePanel({ runs }: { runs: BenchHistoryRow[] }) {
             ...
             {runs.map((run) => (
               <tr key={run.rowid ?? run.id} className="border-b border-neutral-800 hover:bg-neutral-800/50">
                 ...
                 <td className="py-3 text-right font-mono">
                   <span className={run.retention_pct !== undefined && run.retention_pct > 70 ? 'text-green-400' : 'text-yellow-400'}>
                     {run.retention_pct?.toFixed(1) ?? '-'}%
                   </span>
                 </td>
                 <td className="py-3 text-right font-mono">
                   <span className={run.accuracy_pct !== undefined && run.accuracy_pct > 70 ? 'text-green-400' : 'text-yellow-400'}>
