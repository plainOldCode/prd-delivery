...
/* ---------- BenchDetail wrapper — uses useParams correctly ---------- */
function BenchDetailWrapper() {
	const { runId } = useParams<{ runId: string }>();
	return <BenchDetailPage runId={Number(runId)} />;
}

/* ---------- Results redirect — /results/:runId → /bench/:runId ---------- */
function ResultsRedirectWrapper() {
	const { runId } = useParams<{ runId: string }>();
	return <Navigate to={`/bench/${runId}`} replace />;
}
	...
	return (
		<div data-testid="profile-menu" className="relative group">
			...
			<div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
				<button type="button" onClick={() => { logout(); navigate('/'); }} className="w-full text-left px-4 py-3 text-sm text-red-600 hover:bg-red-50 rounded-lg transition">
					Sign Out
				</button>
			</div>
		</div>
		...
		<AuthProvider>
			<BrowserRouter>
				<Routes>
					...
					{/* /results/:runId redirected to /bench/:runId (unified route) */}
					<Route path="/results/:runId" element={<ResultsRedirectWrapper />} />
				</Routes>
			</BrowserRouter>
		</AuthProvider>
	);
