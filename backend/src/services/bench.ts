...
export async function runSpeedBench(model: string, baseUrl = 'http://localhost:11434'): Promise<{ promptTps: number; genTps: number; ttftMs: number }> {
  ...
  if (!resp.ok) throw new Error(`Model endpoint error: ${resp.status}`);
   ...
   // Process streaming response — count tokens for Ollama format
  let fullText = '';
  try {
    ...
    if (reader) {
      ...
      while (true) {
        ...
        fullText += chunk;
         // Ollama streams JSON objects per line with "token" field
        const tokens = chunk.match(/"token"\s*:\s*"[^"]*"/g);
        if (tokens) ttsCount += tokens.length;
       }
     }
   } catch {
     // Fallback: count output size as proxy
    fullText = await resp.text?.() ?? '';
    ttsCount = Math.ceil(fullText.length / 4); // Approximate token count
   }
...
export async function saveBenchRun(run: BenchRun): Promise<void> {
  ...
  const runId = result?.[0]?.rowid ?? run.runId;
...
function generateHaystack(wordCount: number, needle: string, position: string): string {
  ...
  for (let i = 0; i < half; i++) parts.push(fillers[i % fillers.length]);
  parts.push(needle);
  for (let i = half; i < wordCount; i++) parts.push(fillers[i % fillers.length]);
  return parts.join(' ');
}
