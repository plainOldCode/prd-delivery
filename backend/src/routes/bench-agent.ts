...
agent.get('/models/suitable', async (c: any) => {
  ...
  try {
    ...
    return c.json({ runtime, models: minContext > 0 ? all.filter(() => false).concat(all) : all });
  } catch {
...
agent.post('/bench/run/compact', async (c: any) => {
  ...
  try {
    ...
    const composite = computeComposite(speedResult.promptTps, speedResult.genTps, speedResult.ttftMs, 0, 0);
    return c.json({ runId, model, score: { speed: { gen_tps: speedResult.genTps, ttft_ms: speedResult.ttftMs }, retention: 0, accuracy: 0, composite }, recommendation: 'speed-only benchmark completed' }, 201);
  } catch (err) {
...
agent.get('/bench/recent', async (c: any) => {
    ...
    return c.json({
      ...
      runs: runs.map((r: Record<string, unknown>) => ({
        ...
        score: {
          ...
          composite: computeComposite(Number(r.speed_prompt_tps), Number(r.speed_gen_tps), Number(r.speed_ttft_ms), Number(r.retention_pct), Number(r.accuracy_pct)),
        },
      })),
    });
  } catch (err) {
...
agent.get('/bench/recommend', async (c: any) => {
...
agent.post('/bench/score', async (c: any) => {
  const body = await c.req.json();
  ...
  try {
    ...
    const composite = computeComposite(Number(run.speed_prompt_tps), Number(run.speed_gen_tps), Number(run.speed_ttft_ms), Number(run.retention_pct), Number(run.accuracy_pct), weights as any);
    return c.json({ runId: run.id, model: run.model_name, score: { speed: { gen_tps: run.speed_gen_tps, ttft_ms: run.speed_ttft_ms }, retention_pct: run.retention_pct, accuracy_pct: run.accuracy_pct, composite, weights } });
  } catch (err) {
...
agent.post('/bench/compare', async (c: any) => {
  const body = await c.req.json();
  ...
  try {
    ...
    return c.json({ count: results.length, compared: ids.length - results.length > 0 ? `${ids.length - results.length} not found` : 'all found', results });
  } catch (err) {
